...
public class CoapOSExceptionHandler {
		...
		if (request != null) {
			if (request.getType() != null) {
				if (ResponseCode.isClientError(responseCode)) {
					...
					org.eclipse.californium.elements.MyLogger.LOG_error(ErrorDescriptions.CANNOT_CREATE_ERROR_MESS + ": " + ErrorDescriptions.ERROR_MESS_NULL);
				}
			} else {
				org.eclipse.californium.elements.MyLogger.LOG_error(ErrorDescriptions.CANNOT_CREATE_ERROR_MESS + ": " + ErrorDescriptions.TYPE_NULL);
			}
		} else {
			org.eclipse.californium.elements.MyLogger.LOG_error(ErrorDescriptions.CANNOT_CREATE_ERROR_MESS + ": " + ErrorDescriptions.REQUEST_NULL);
		}
		return null;
	}
	...
	public static EmptyMessage manageError(OSException e, Response response) {
		if (e == null) {
			org.eclipse.californium.elements.MyLogger.LOG_error(ErrorDescriptions.EXCEPTION_NULL);
		...
		if (errMess == null) {
			org.eclipse.californium.elements.MyLogger.LOG_error(ErrorDescriptions.ERROR_MESS_NULL);
			throw new NullPointerException(ErrorDescriptions.ERROR_MESS_NULL);
		}
		
		if (!response.isConfirmable()) {
			org.eclipse.californium.elements.MyLogger.LOG_error("An Empty Message will not be created");
			return null;
		}
		
		org.eclipse.californium.elements.MyLogger.LOG_info("Sending empty RST message");
