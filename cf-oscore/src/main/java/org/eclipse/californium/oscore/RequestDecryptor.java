...
public class RequestDecryptor extends Decryptor {
	...
	public static Request decrypt(OSCoreCtxDB db, Request request) throws CoapOSException {
		...
		org.eclipse.californium.elements.MyLogger.LOG_info("Removes E options from outer options which are not allowed there");
		...
		try {
			...
			org.eclipse.californium.elements.MyLogger.LOG_error(ErrorDescriptions.FAILED_TO_DECODE_COSE);
		...
		if (kid == null || !kid.getType().equals(CBORType.ByteString)) {
			org.eclipse.californium.elements.MyLogger.LOG_error(ErrorDescriptions.MISSING_KID);
			throw new CoapOSException(ErrorDescriptions.FAILED_TO_DECODE_COSE, ResponseCode.BAD_OPTION);
		}
		...
		if (ctx == null) {
			org.eclipse.californium.elements.MyLogger.LOG_error(ErrorDescriptions.CONTEXT_NOT_FOUND);
		...
		try {
			...
			if (e.getMessage().equals(ErrorDescriptions.REPLAY_DETECT)) { 
				org.eclipse.californium.elements.MyLogger.LOG_error(ErrorDescriptions.REPLAY_DETECT);
			...
			org.eclipse.californium.elements.MyLogger.LOG_error(ErrorDescriptions.DECRYPTION_FAILED);
