		...
		if (shouldProtectRequest(request)) {
			try {
				...
				if (ctx.getContextRederivationPhase() == PHASE.CLIENT_INITIATE) {
						...
						public void onResponse(final Response response) {
							try {
								...
								if (ctx == null) {
									...
									org.eclipse.californium.elements.MyLogger.LOG_error("Expected phase 2, but is {}", ctx.getContextRederivationPhase());
								}
							} catch (OSException e) {
							}
							...
							exchange.execute(new Runnable() {
								...
								@Override
								public void run() {
									org.eclipse.californium.elements.MyLogger.LOG_info("Original Request: " + exchange.getRequest().toString());
					...
					org.eclipse.californium.elements.MyLogger.LOG_info("Auxiliary Request: " + exchange.getRequest().toString());
