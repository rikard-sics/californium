...
public abstract class Decryptor {
	...
	protected static byte[] decryptAndDecode(Encrypt0Message enc, Message message, OSCoreCtx ctx, Integer seqByToken)
		...
		if (isRequest) {
			...
			if (piv == null) {
				org.eclipse.californium.core.MyLogger.LOG_error("Decryption failed: no partialIV in request");
			...
			if (seqByToken == null) {
				org.eclipse.californium.core.MyLogger.LOG_error("Decryption failed: the arrived response is not connected to a request we sent");
				throw new OSException(ErrorDescriptions.DECRYPTION_FAILED);
			}
		...
		try {
			...
			org.eclipse.californium.core.MyLogger.LOG_error(ErrorDescriptions.DECRYPTION_FAILED + " " + e.getMessage());
	...
	private static byte[] expandToIntSize(byte[] partialIV) throws OSException {
		if (partialIV.length > INTEGER_BYTES) {
			org.eclipse.californium.core.MyLogger.LOG_error("The partial IV is: " + partialIV.length + " long, " + INTEGER_BYTES + " was expected");
	...
	protected static Encrypt0Message decompression(byte[] cipherText, Message message) throws OSException {
		...
		try {
			...
			org.eclipse.californium.core.MyLogger.LOG_error(e.getMessage());
			throw e;
		} catch (Exception e) {
			org.eclipse.californium.core.MyLogger.LOG_error("Failed to decode object security option.");
	...
	private static void decodeObjectSecurity(Message message, Encrypt0Message enc) throws OSException {
		...
		if (n > 0) {
			try {
				...
				org.eclipse.californium.core.MyLogger.LOG_error("Partial_IV is missing from message when it is expected.");
		...
		if (h != 0) {
			...
			if (s > 0) {
				org.eclipse.californium.core.MyLogger.LOG_info("Received KID Context: " + Utils.toHexString(kidContext));
			} else {
				org.eclipse.californium.core.MyLogger.LOG_error("Kid context is missing from message when it is expected.");
				throw new OSException(ErrorDescriptions.FAILED_TO_DECODE_COSE);
			}
		}
		...
		if (k != 0) {
			...
			if (message instanceof Request) {
				org.eclipse.californium.core.MyLogger.LOG_error("Kid is missing from message when it is expected.");
				throw new OSException(ErrorDescriptions.FAILED_TO_DECODE_COSE);
			}
		}
		...
		try {
			...
			org.eclipse.californium.core.MyLogger.LOG_error("COSE processing of message failed.");
			e.printStackTrace();
		}
	}
