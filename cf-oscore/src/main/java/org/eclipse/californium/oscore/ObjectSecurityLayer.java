...
public class ObjectSecurityLayer extends AbstractLayer {
	...
	/**
	 * Check if a response has a valid response code for OSCORE. Only
	 * unprotected responses with one of these response codes should
	 * be accepted. Additional codes are allowed if the request used
	 * a proxy.
	 *
	...
	private boolean hasValidResponseCode(ResponseCode responseCode, Request request) {
		...
		boolean requestUsedProxy = request != null
				&& (request.hasProxyURI() || request.getOptions().getProxyScheme() != null);
