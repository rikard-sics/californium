...
public class ObjectSecurityLayer extends AbstractLayer {
	...
	public void sendResponse(Exchange exchange, Response response) {
		...
		if (shouldProtectResponse(exchange)) {
			try {
				...
				org.eclipse.californium.core.MyLogger.LOG_error("Error sending response: " + e.getMessage());
				return;
			}
		}
	...
	public void receiveRequest(Exchange exchange, Request request) {
		if (isProtected(request)) {
			...
			try {
				...
				org.eclipse.californium.core.MyLogger.LOG_error("Error while receiving OSCore request: " + e.getMessage());
				Response error;
	...
	public void receiveResponse(Exchange exchange, Response response) {
		...
		if (request == null) {
			org.eclipse.californium.core.MyLogger.LOG_error("No request tied to this response");
			return;
		}
		try {
			...
			if (!isProtected(response) && responseShouldBeProtected(exchange, response)) {
				org.eclipse.californium.core.MyLogger.LOG_warn("Incoming response is NOT OSCORE protected!");
			} else if (isProtected(response)) {
				org.eclipse.californium.core.MyLogger.LOG_info("Incoming response is OSCORE protected");
			}
			...
			org.eclipse.californium.core.MyLogger.LOG_error("Error while receiving OSCore response: " + e.getMessage());
			...
			if (response.getOptions().hasObserve() && request.getOptions().hasObserve()) {
				...
				if (options.hasOscore()) {
					...
					try {
						...
						org.eclipse.californium.core.MyLogger.LOG_error("Error when re-creating exchange at OSCORE level");
