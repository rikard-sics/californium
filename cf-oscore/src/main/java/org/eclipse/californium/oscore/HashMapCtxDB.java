...
public class HashMapCtxDB implements OSCoreCtxDB {
	...
	public synchronized OSCoreCtx getContext(String uri) throws OSException {
		if (uri != null) {
			...
			org.eclipse.californium.core.MyLogger.LOG_error(ErrorDescriptions.STRING_NULL);
	...
	public synchronized void addContext(OSCoreCtx ctx) {
		if (ctx != null) {
			...
			org.eclipse.californium.core.MyLogger.LOG_error(ErrorDescriptions.CONTEXT_NULL);
	...
	public synchronized void addSeqByToken(Token token, Integer seq) {
		...
		if (tokenExist(token)) {
			org.eclipse.californium.core.MyLogger.LOG_info("Token exists, but this could be a refresh if not there is a problem");
		} else {
			allTokens.add(token);
		}
	...
	private static String normalizeServerUri(String uri) throws OSException {
		...
		try {
			...
			try {
				...
				org.eclipse.californium.core.MyLogger.LOG_error("Error in the request URI: " + uri + " message: " + e.getMessage());
		...
		try {
			...
			org.eclipse.californium.core.MyLogger.LOG_error("Error finding host of request URI: " + uri + " message: " + e.getMessage());
		}
