...
public class OSCoreCtx {
	...
	public OSCoreCtx(byte[] master_secret, boolean client, AlgorithmID alg, byte[] sender_id, byte[] recipient_id,
		...
		if (master_secret != null) {
			...
			org.eclipse.californium.core.MyLogger.LOG_error("Input master secret is null");
		...
		default:
			org.eclipse.californium.core.MyLogger.LOG_error("Requested HKDF algorithm is not supported: " + this.kdf.toString());
	...
	public void setIncludeContextId(boolean includeContextId) {
		if (context_id == null && overrideContextId == null) {
			org.eclipse.californium.core.MyLogger.LOG_error("Context ID cannot be included for a context without one set.");
	...
	private void setLengths() {
		if (common_alg != null) {
			if (common_alg.equals(AlgorithmID.AES_CCM_16_64_128)) {
				...
				org.eclipse.californium.core.MyLogger.LOG_error("Unable to set lengths, since algorithm");
				throw new RuntimeException("Unable to set lengths, since algorithm");
			}
		} else {
			org.eclipse.californium.core.MyLogger.LOG_error("Common_alg has not yet been initiated.");
	...
	public synchronized void increaseSenderSeq() throws OSException {
		if (sender_seq >= seqMax) {
			org.eclipse.californium.core.MyLogger.LOG_error("Sequence number wrapped, get a new OSCore context");
	...
	public synchronized void checkIncomingSeq(int seq) throws OSException {
		if (seq >= seqMax) {
			org.eclipse.californium.core.MyLogger.LOG_error("Sequence number wrapped, get new OSCore context");
		...
		if (seq > recipient_seq) {
			...
			org.eclipse.californium.core.MyLogger.LOG_error("Sequence number is replay");
			throw new OSException(ErrorDescriptions.REPLAY_DETECT);
		} else { // seq < recipient_seq
			if (seq + recipient_replay_window_size < recipient_seq) {
				org.eclipse.californium.core.MyLogger.LOG_error("Message too old");
				throw new OSException(ErrorDescriptions.REPLAY_DETECT);
			}
