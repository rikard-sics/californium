	...
	public static byte[] serializeConfidentialData(OptionSet options, byte[] payload, int realCode) {
		if (options != null) {
			...
			if (realCode > 0) {
				...
				org.eclipse.californium.elements.MyLogger.LOG_error(ErrorDescriptions.COAP_CODE_INVALID);
				throw new IllegalArgumentException(ErrorDescriptions.COAP_CODE_INVALID);
			}
		} else {
			org.eclipse.californium.elements.MyLogger.LOG_error(ErrorDescriptions.OPTIONSET_NULL);
		...
		if (version == CoAP.VERSION) {
			if (requestSeq > -1) {
				if (algorithm != null) {
					...
					org.eclipse.californium.elements.MyLogger.LOG_error(ErrorDescriptions.ALGORITHM_NOT_DEFINED);
					throw new NullPointerException(ErrorDescriptions.ALGORITHM_NOT_DEFINED);
				}
			} else {
				org.eclipse.californium.elements.MyLogger.LOG_error(ErrorDescriptions.SEQ_NBR_INVALID);
				throw new IllegalArgumentException(ErrorDescriptions.SEQ_NBR_INVALID);
			}
		} else {
			org.eclipse.californium.elements.MyLogger.LOG_error(ErrorDescriptions.WRONG_VERSION_NBR);
			...
			if (senderID != null) {
				if (commonIV != null) {
					if (nonceLength > 0) {
						...
						org.eclipse.californium.elements.MyLogger.LOG_error(ErrorDescriptions.NONCE_LENGTH_INVALID);
						throw new IllegalArgumentException(ErrorDescriptions.NONCE_LENGTH_INVALID);
					}
				} else {
					org.eclipse.californium.elements.MyLogger.LOG_error(ErrorDescriptions.COMMON_IV_NULL);
					throw new NullPointerException(ErrorDescriptions.COMMON_IV_NULL);
				}
			} else {
				org.eclipse.californium.elements.MyLogger.LOG_error(ErrorDescriptions.SENDER_ID_NULL);
				throw new NullPointerException(ErrorDescriptions.SENDER_ID_NULL);
			}
		} else {
			org.eclipse.californium.elements.MyLogger.LOG_error(ErrorDescriptions.PARTIAL_IV_NULL);
