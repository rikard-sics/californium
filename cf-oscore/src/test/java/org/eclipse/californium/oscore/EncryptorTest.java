...
public class EncryptorTest {
	...
	public void testResponseEncryptor() throws OSException {
		...
		ctx.setResponsesIncludePartialIV(true);
		...
		// Set up state information simulating the original outgoing request
		OSCoreCtxDB db = new HashMapCtxDB();
		db.addContext(ctx);

		//Encrypt the response message
		// OSCoreCtxDB db, Response response, byte[] contextIdentifier, boolean
		// outerBlockwise)
		boolean newPartialIV = true;
		Response encrypted = ResponseEncryptor.encrypt(db, r, ctx.getUniqueIdentifier(), false);
