...
public class KudosTest {
	...
	/**
	 * Tests the decoding of the OSCORE option for the parts relevant to KUDOS.
	 * With z=1 (convergent message flag).
	 */
	@Test
	public void testKudosOscoreOptionDecoding2() throws CoapOSException {

		// Same as test 1 but with x=0x47 (z=1 convergent flag set)
		byte[] oscoreOption = StringUtil.hex2ByteArray("9A0115B3030A0B0C470001020304050607AA");
		byte[] correctKid = new byte[] { (byte) 0xAA };
		int correctSenderSequenceNumber = 5555;
		byte[] correctIdContext = new byte[] { (byte) 0x0A, 0x0B, 0x0C };
		byte[] correctNonce = new byte[] { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 };
	...
	public void testKudosOscoreOptionEncoding2() throws CoapOSException {
		...
		// Same as test 1 but with z=1 set
		byte[] correctOption = StringUtil.hex2ByteArray("9A0115B3030A0B0C470001020304050607AA");
		byte[] kid = new byte[] { (byte) 0xAA };
		int senderSequenceNumber = 5555;
		byte[] idContext = new byte[] { (byte) 0x0A, 0x0B, 0x0C };
		byte[] nonce = new byte[] { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 };
	...
	/**
	 * Test context KUDOS re-derivation initiated by the client, followed by a
	 * normal message exchange. This test simulates a client losing the mutable
	 * parts of the OSCORE context, and then explicitly initiating the context
	...
	public void testClientInitiatedRederivation()
		...
		// Check contents of OSCORE option (convergent response has z=1)
		OscoreOptionDecoder decoder = new OscoreOptionDecoder(resp.getOptions().getOscore());
		assertEquals("Invalid Z bit value", 1, decoder.getZ());
	 ...
	 * This test simulates a server losing the mutable parts of the OSCORE
	 * context. When a request from the client arrives this will make the server
	 * initiate the KUDOS context re-derivation procedure. Note that the client
	 * does not explicitly initiate the procedure before the request as it still
	 * has the context information. It does not know the server has lost this
	 * information.
	 * 
	 * @throws OSException on failure
	 * @throws ConnectorException on failure
	...
	public void testServerInitiatedRederivation()
		...
		CoapClient client = new CoapClient(serverUri + kudosResource);
		Request request = new Request(Code.GET);
		request.getOptions().setOscore(Bytes.EMPTY);
		CoapResponse resp = client.advanced(request);
		System.out.println((Utils.prettyPrint(resp)));
		...
		OSCoreCtx currCtx = dbClient.getContext(serverUri);
		assertEquals(ContextRederivation.PHASE.KUDOS_CLIENT_PHASE2, currCtx.getContextRederivationPhase()); // Phase
		...
		// The response should be a 4.04
		assertEquals(ResponseCode.NOT_FOUND, resp.getCode());

		// 2nd request (for request #2 and response #2 exchange)
		client = new CoapClient(serverUri + hello1);
		request = new Request(Code.GET);
		request.setURI(serverUri + hello1);
		request.getOptions().setOscore(Bytes.EMPTY);
		resp = client.advanced(request);
		System.out.println((Utils.prettyPrint(resp)));
