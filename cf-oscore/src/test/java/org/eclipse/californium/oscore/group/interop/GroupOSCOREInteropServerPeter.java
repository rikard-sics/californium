...
public class GroupOSCOREInteropServerPeter {
	...
	private final static byte[] master_secret = InteropParametersOld.RIKARD_MASTER_SECRET_EDDSA;
	private final static byte[] master_salt = InteropParametersOld.RIKARD_MASTER_SALT_EDDSA;

	private static final int REPLAY_WINDOW = 32;

	// Public and private keys for group members

	private static byte[] sid = InteropParametersOld.RIKARD_ENTITY_2_KID_EDDSA;
	private static OneKey sid_private_key;

	private final static byte[] rid1 = InteropParametersOld.RIKARD_ENTITY_1_KID_EDDSA;
	...
	public static void main(String[] args) throws Exception {
		...
		sid_private_key = OneKeyDecoder.parseDiagnostic(InteropParametersOld.RIKARD_ENTITY_2_KEY_EDDSA);
		rid1_public_key = OneKeyDecoder.parseDiagnostic(InteropParametersOld.PETER_PUBLIC_KEY_EDDSA);
		...
		if (args.length != 0) {
			...
			sid_private_key = OneKeyDecoder.parseDiagnostic(InteropParametersOld.RIKARD_ENTITY_2_KEY_ECDSA); // Unused
		} else {
