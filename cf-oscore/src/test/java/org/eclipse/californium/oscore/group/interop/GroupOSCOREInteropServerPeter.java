 ...
 */
public class GroupOSCOREInteropServerPeter {

	private static final Logger LOGGER = LoggerFactory.getLogger(GroupOSCOREInteropServerPeter.class);
	...
	private final static byte[] master_secret = InteropParametersNew.RIKARD_MASTER_SECRET_EDDSA;
	private final static byte[] master_salt = InteropParametersNew.RIKARD_MASTER_SALT_EDDSA;

	private static final int REPLAY_WINDOW = 32;

	// Public and private keys for group members

	private static byte[] sid = InteropParametersNew.RIKARD_ENTITY_2_KID_EDDSA;
	private static OneKey sid_private_key;

	private final static byte[] rid1 = InteropParametersNew.RIKARD_ENTITY_1_KID_EDDSA;
	private static OneKey rid1_public_key;

	// FIXME
	// private final static byte[] group_identifier =
	// InteropParametersNew.RIKARD_GROUP_ID_EDDSA;
	private final static byte[] group_identifier = null;
	...
	public static void main(String[] args) throws Exception {
		...
		sid_private_key = OneKeyDecoder.parseDiagnostic(InteropParametersNew.RIKARD_ENTITY_2_KEY_EDDSA);
		rid1_public_key = OneKeyDecoder.parseDiagnostic(InteropParametersNew.PETER_PUBLIC_KEY_EDDSA);
		...
		if (args.length != 0) {
			...
			sid_private_key = OneKeyDecoder.parseDiagnostic(InteropParametersNew.RIKARD_ENTITY_2_KEY_ECDSA); // Unused
		} else {
