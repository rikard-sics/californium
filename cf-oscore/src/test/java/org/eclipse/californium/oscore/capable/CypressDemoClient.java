...
/**
 * 
 * Demo client for CYPRESS showing the OSCORE-capable proxies functionalities.
 * Run together with OscoreCapableProxy and OscoreCapableServer
 *
 */
public class CypressDemoClient {
	...
	private final static int[][] optionSetsURI = { {}, { OptionNumberRegistry.PROXY_URI } };

	private final static boolean[] ProxyURIAnswer = { true, true, true, true, false };

	private final static boolean[][][] answerSetsURI = { {}, { ProxyURIAnswer } };

	private final static CBORObject[][] postValuesUri = { { CBORObject.FromObject(uriServer + uriServerPath) }, {} };

	private final static int[][] optionSetsPostUri = { { OptionNumberRegistry.PROXY_URI }, {} };
	...
	public static void main(String[] args) throws OSException, ConnectorException, IOException, InterruptedException {
		OSCoreCtx ctxserver = new OSCoreCtx(master_secret, true, alg, sids[0], rids[0], kdf, 32, master_salt,
				idcontexts[0], MAX_UNFRAGMENTED_SIZE);
		db.addContext(uriServer, ctxserver);

		OSCoreCtx ctxproxy = new OSCoreCtx(master_secret, true, alg, sids[1], rids[1], kdf, 32, master_salt,
				idcontexts[1], MAX_UNFRAGMENTED_SIZE);
	...
	public static byte[] getPostURIInstruction() {
		...
		for (int i = 0; i < rids.length; i++) {
			instructionsUri = Bytes.concatenate(instructionsUri, OptionEncoder.set(rids[i], idcontexts[i],
					optionSetsURI[i], answerSetsURI[i], optionSetsPostUri[i], postValuesUri[i]));
		}
	...
	public static byte[] getPostSchemeInstruction() {
		...
		for (int i = 0; i < rids.length; i++) {
			instructionsScheme = Bytes.concatenate(instructionsScheme, OptionEncoder.set(rids[i], idcontexts[i],
					optionSetsScheme[i], answerSetsScheme[i], optionSetsPostScheme[i], postValuesScheme[i]));
		}
		return instructionsScheme;
	}

	public static CoapResponse PostObserveSend(CoapClient c)
