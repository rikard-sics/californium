...
public class EdhocSession {
	 ...
	 * @param context   The context to use to derive the OKM, as a CBOR sequence (or Null as an empty CBOR sequence)
	 ...
	 */
	public byte[] edhocKDF(byte[] prk, byte[] transcript_hash, String label, byte[] context, int len)
			throws InvalidKeyException, NoSuchAlgorithmException {
		
		if (prk == null || transcript_hash == null || label == null)
			return null;
		...
		if (context != null) {
			CBORObject[] contextElements = null;
			try {
				contextElements = CBORObject.DecodeSequenceFromBytes(context);
			}
			catch (Exception e) {
			    return null;
			}
			...
			if (contextElements != null) {
				for (int i = 0; i < contextElements.length; i++)
					infoArray.Add(contextElements[i]);
			}
		}
