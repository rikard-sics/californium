...
public class EdhocSession {
	...
	public byte[] edhocExporter(String label, byte[] context, int len) throws InvalidKeyException, NoSuchAlgorithmException {
		...
		CBORObject labelCBOR = null;
		if (label != null) {
			labelCBOR = CBORObject.FromObject(label);
		}
		else {
			labelCBOR = CBORObject.FromObject("");
		}
		...
		objectList.add(labelCBOR);
		objectList.add(contextCBOR);
		byte[] labelContext = Util.buildCBORSequence(objectList);
		
		return edhocKDF(this.prk_4x3m, this.TH4, labelContext, len);
	 ...
	 * @param labelContext   The label and context to use to derive the OKM, as a CBOR sequence
	 * @param len   The intended length of the OKM to derive, in bytes
	 * @return  the OKM generated by HKDF-Expand
	 */
	public byte[] edhocKDF(byte[] prk, byte[] transcript_hash, byte[] labelContext, int len) 
		...
		try {
			contextSeqElements = CBORObject.DecodeSequenceFromBytes(labelContext);
		}
		catch (Exception e) {
		    return null;
		}
