...
public class Phase1Client {
	...
	private static long edhocTotal;
	...
	// Set to True if this CoAP client is the EDHOC initiator (only flow
	// available at the moment)
	// Relevant to choose with public keys to install, when testing with
	// selected ciphersuite 0 or 1
	private final static boolean isInitiator = true;

	// What will be the selected ciphersuite as indicated in EDHOC message 1
	// Has to be aligned between Initiator and Responder, to choose which public
	// keys to install for testing
	...
	// Set to true if an OSCORE-protected exchange is performed after EDHOC
	// completion
	...
	// request
	// Note: the application profile pertaining the EDHOC resource must first
	// indicate support for the combined request
	...
	// Connection Identifiers are stored as CBOR integers (if numeric) or as
	// CBOR byte strings (if binary)
	...
	// The collection of application profiles - The lookup key is the full URI
	// of the EDHOC resource
	...
	// "coap://51.75.194.248/.well-known/edhoc"; // Timothy
	// private static String edhocURI = "coap://54.93.59.163/.well-known/edhoc";
	// // Stefan
	// private static String edhocURI =
	// "coap://195.251.58.203:5683/.well-known/edhoc"; // Lidia
	// private static String edhocURI =
	// "coap://hephaistos.proxy.rd.coap.amsuess.com:1234/.well-known/edhoc"; //
	// Christian
	...
	public static void main(String args[]) throws InterruptedException, ConnectorException, IOException {
		...
		boolean supportCombinedRequest = true; // If set to true, it overrides
												// the ID conversion method to
												// CONVERSION_ID_CORE
		int conversionMethodOscoreToEdhoc = Constants.CONVERSION_ID_UNDEFINED; // Undefined
																				// yields
																				// using
																				// CONVERSION_ID_CORE
		AppProfile appProfile = new AppProfile(authMethods, useMessage4, usedForOSCORE, supportCombinedRequest,
				conversionMethodOscoreToEdhoc);
		...
		// Possibly specify external authorization data for EAD_1, or null if
		// none has to be provided
		// The EAD is structured in pairs of CBOR items (int, any), i.e. the EAD
		// Label first and then the EAD Value
	...
	private static void setupIdentityKeys() {
		...
		// In order to install the right public keys to use for testing,
		// determine the public key curve
		// consistent with the expected selected cipher suite and the expected
		...
		if (keyCurve == KeyKeys.EC2_P256.AsInt32()) {
			privateKeyBinary = net.i2p.crypto.eddsa.Utils
					.hexToBytes("04f347f2bead699adb247344f347f2bdac93c7f2bead6a9d2a9b24754a1e2b62");
			publicKeyBinary = net.i2p.crypto.eddsa.Utils
					.hexToBytes("cd4177ba62433375ede279b5e18e8b91bc3ed8f1e174474a26fc0edb44ea5373");
			publicKeyBinaryY = net.i2p.crypto.eddsa.Utils
					.hexToBytes("A0391DE29C5C5BADDA610D4E301EAAA18422367722289CD18CBE6624E89B9CFD");
		}
		else if (keyCurve == KeyKeys.OKP_Ed25519.AsInt32()) {
			privateKeyBinary = net.i2p.crypto.eddsa.Utils
					.hexToBytes("2ffce7a0b2b825d397d0cb54f746e3da3f27596ee06b5371481dc0e012bc34d7");
			publicKeyBinary = net.i2p.crypto.eddsa.Utils
					.hexToBytes("38e5d54563c2b6a4ba26f3015f61bb706e5c2efdb556d2e1690b97fc3c6de149");
		} else if (keyCurve == KeyKeys.OKP_X25519.AsInt32()) {
			privateKeyBinary = net.i2p.crypto.eddsa.Utils
					.hexToBytes("2bbea655c23371c329cfbd3b1f02c6c062033837b8b59099a4436f666081b08e");
			publicKeyBinary = net.i2p.crypto.eddsa.Utils
					.hexToBytes("2c440cc121f8d7f24c3b0e41aedafe9caa4f4e7abb835ec30f1de88adb96ff71");
		}

		if (peerKeyCurve == KeyKeys.EC2_P256.AsInt32()) {
			peerPublicKeyBinary = net.i2p.crypto.eddsa.Utils
					.hexToBytes("6f9702a66602d78f5e81bac1e0af01f8b52810c502e87ebb7c926c07426fd02f");
			peerPublicKeyBinaryY = net.i2p.crypto.eddsa.Utils
					.hexToBytes("C8D33274C71C9B3EE57D842BBF2238B8283CB410ECA216FB72A78EA7A870F800");
		}
		else if (peerKeyCurve == KeyKeys.OKP_Ed25519.AsInt32()) {
			peerPublicKeyBinary = net.i2p.crypto.eddsa.Utils
					.hexToBytes("dbd9dc8cd03fb7c3913511462bb23816477c6bd8d66ef5a1a070ac854ed73fd2");
		} else if (peerKeyCurve == KeyKeys.OKP_X25519.AsInt32()) {
			peerPublicKeyBinary = net.i2p.crypto.eddsa.Utils
					.hexToBytes("a3ff263595beb377d1a0ce1d04dad2d40966ac6bcb622051b84659184d5d9a32");
		}
		...
		case Constants.CRED_TYPE_CCS:
			...
			// These serializations have to be prepared manually, in order to
			// ensure that
			// the CBOR map used as CRED has its parameters encoded in bytewise
			// lexicographic order
			if (keyCurve == KeyKeys.EC2_P256.AsInt32()) {
				cred = net.i2p.crypto.eddsa.Utils.hexToBytes(
						"a2026008a101a501020241242001215820cd4177ba62433375ede279b5e18e8b91bc3ed8f1e174474a26fc0edb44ea5373225820a0391de29c5c5badda610d4e301eaaa18422367722289cd18cbe6624e89b9cfd");
			} else if (keyCurve == KeyKeys.OKP_Ed25519.AsInt32()) {
				cred = net.i2p.crypto.eddsa.Utils.hexToBytes(
						"a2026008a101a40101024124200621582038e5d54563c2b6a4ba26f3015f61bb706e5c2efdb556d2e1690b97fc3c6de149");
			} else if (keyCurve == KeyKeys.OKP_X25519.AsInt32()) {
				cred = net.i2p.crypto.eddsa.Utils.hexToBytes(
						"a2026008a101a4010102412420042158202c440cc121f8d7f24c3b0e41aedafe9caa4f4e7abb835ec30f1de88adb96ff71");
			}
			break;
		case Constants.CRED_TYPE_X509:
			// The x509 certificate of this peer
			serializedCert = net.i2p.crypto.eddsa.Utils.hexToBytes(
					"5413204c3ebc3428a6cf57e24c9def59651770449bce7ec6561e52433aa55e71f1fa34b22a9ca4a1e12924eae1d1766088098449cb848ffc795f88afc49cbe8afdd1ba009f21675e8f6c77a4a2c30195601f6f0a0852978bd43d28207d44486502ff7bdda6");
			...
			// CRED, as serialization of a CBOR byte string wrapping the
		...
		case Constants.ID_CRED_TYPE_KID:
			// ID_CRED for the identity key of this peer, using the kid
			// associated to the RPK
			idCred = Util.buildIdCredKid(idCredKid);
			break;
		case Constants.ID_CRED_TYPE_CWT:
			// ID_CRED for the identity key of this peer, using a CWT to
			// transport the RPK by value
			// TODO
			break;
		case Constants.ID_CRED_TYPE_CCS:
			// ID_CRED for the identity key of this peer, using a CCS to
			// transport the RPK by value
			idCred = Util.buildIdCredKccs(ccsObject);
			break;
		case Constants.ID_CRED_TYPE_X5CHAIN:
			// ID_CRED for the identity key of this peer, built from the x509
			// certificate using x5chain
			idCred = Util.buildIdCredX5chain(serializedCert);
			break;
		case Constants.ID_CRED_TYPE_X5T:
			// ID_CRED for the identity key of this peer, built from the x509
			// certificate using x5t
			idCred = Util.buildIdCredX5t(serializedCert);
			break;
		case Constants.ID_CRED_TYPE_X5U:
			// ID_CRED for the identity key of this peer, built from the x509
			// certificate using x5u
		...
		/* Values as binary serializations */
		if (peerKeyCurve == KeyKeys.EC2_P256.AsInt32()) {
			peerPublicKey = SharedSecretCalculation.buildEcdsa256OneKey(null, peerPublicKeyBinary,
		...
		case Constants.CRED_TYPE_CCS:
			...
			peerCcsObject = CBORObject
					.DecodeFromBytes(Util.buildCredRawPublicKeyCcs(peerPublicKey, subjectName, peerKidCbor));
			...
			if (peerKeyCurve == KeyKeys.EC2_P256.AsInt32()) {
				peerCred = net.i2p.crypto.eddsa.Utils.hexToBytes(
						"a2026008a101a5010202410720012158206f9702a66602d78f5e81bac1e0af01f8b52810c502e87ebb7c926c07426fd02f225820c8d33274c71c9b3ee57d842bbf2238b8283cb410eca216fb72a78ea7a870f800");
			}
			else if (peerKeyCurve == KeyKeys.OKP_Ed25519.AsInt32()) {
				peerCred = net.i2p.crypto.eddsa.Utils.hexToBytes(
						"a2026008a101a401010241072006215820dbd9dc8cd03fb7c3913511462bb23816477c6bd8d66ef5a1a070ac854ed73fd2");
			} else if (peerKeyCurve == KeyKeys.OKP_X25519.AsInt32()) {
				peerCred = net.i2p.crypto.eddsa.Utils.hexToBytes(
						"a2026008a101a401010241072004215820a3ff263595beb377d1a0ce1d04dad2d40966ac6bcb622051b84659184d5d9a32");
			}
			break;
		case Constants.CRED_TYPE_X509:
			// The x509 certificate of the other peer
			peerSerializedCert = net.i2p.crypto.eddsa.Utils.hexToBytes(
					"c788370016b8965bdb2074bff82e5a20e09bec21f8406e86442b87ec3ff245b70a47624dc9cdc6824b2a4c52e95ec9d6b0534b71c2b49e4bf9031500cee6869979c297bb5a8b381e98db714108415e5c50db78974c271579b01633a3ef6271be5c225eb2");
		...
		case Constants.ID_CRED_TYPE_KID:
			// ID_CRED for the identity key of the other peer, using the kid
			// associated to the RPK
			peerIdCred = Util.buildIdCredKid(peerKid);
			break;
		case Constants.ID_CRED_TYPE_CWT:
			// ID_CRED for the identity key of the other peer, using a CWT to
			// transport the RPK by value
			// TODO
			break;
		case Constants.ID_CRED_TYPE_CCS:
			// ID_CRED for the identity key of the other peer, using a CCS to
			// transport the RPK by value
			peerIdCred = Util.buildIdCredKccs(peerCcsObject);
			break;
		case Constants.ID_CRED_TYPE_X5CHAIN:
			// ID_CRED for the identity key of the other peer, built from the
			// x509 certificate using x5chain
			peerIdCred = Util.buildIdCredX5chain(peerSerializedCert);
			break;
		case Constants.ID_CRED_TYPE_X5T:
			// ID_CRED for the identity key of the other peer, built from the
			// x509 certificate using x5t
		...
		if (response != null) {
			...
			if (args.length > 1) {
				try (FileOutputStream out = new FileOutputStream(args[1])) {
					...
					// System.err.println("Error while writing the response
					// payload to file: " + e.getMessage());
				}
			} else {
	...
	private static void edhocExchangeAsInitiator(final String args[], final URI targetUri, Set<CBORObject> ownIdCreds,
		 ...
		 * // Submit the request //
		 * System.out.println("\nSent EDHOC Message1\n"); CoapResponse
		 * edhocMessage2; try { edhocMessage2 = client.advanced(edhocMessage1);
		...
		// System.out.println("Phase 1 Client ready to execute EDHOC and send
		// following OSCORE request." + "\n");
		...
		// At this point, the initiator may overwrite the information in the
		// EDHOC session about the supported ciphersuites
		// and the selected ciphersuite, based on a previously received EDHOC
		// Error Message
		...
		if (nextPayload == null || session.getCurrentStep() != Constants.EDHOC_BEFORE_M1) {
			// System.err.println("Inconsistent state before sending EDHOC
			// Message 1");
		...
		try {
			...
			// System.err.println("ConnectorException when sending EDHOC Message
			// 1");
		...
		// Hence, the Initiator knows what session to refer to, from which the
		// correct C_I can be retrieved
		...
		if (responseType == Constants.EDHOC_ERROR_MESSAGE) {
			...
			if (objectList != null) {
				...
				// Hence, there is no C_I included, and the first element of the
				// EDHOC Error Message is ERR_CODE.
				...
				// The following simply deletes the EDHOC session. However, it
				// would be fine to prepare a new
				// EDHOC Message 1 right away, keeping the same Connection
				// Identifier C_I and this same session.
				// In fact, the session is marked as "used", hence new ephemeral
				// keys would be generated when
		...
		if (responseType == Constants.EDHOC_MESSAGE_2) {
			...
			// Possibly specify external authorization data for EAD_3, or null
			// if none has to be provided
			// The EAD is structured in pairs of CBOR items (int, any), i.e. the
			// EAD Label first and then the EAD Value
			...
			if (processingResult.get(0) == null || processingResult.get(0).getType() != CBORType.ByteString) {
				// System.err.println("Internal error when processing EDHOC
				// Message 2");
				Util.purgeSession(session, connectionId, edhocSessions, usedConnectionIds);
				client.shutdown();
				return;
			...
			// A non-zero length response payload would be an EDHOC Error
			// Message
			...
			if (nextPayload.length == 0) {
				...
				if (processingResult.size() == 2 && processingResult.get(1).getType() == CBORType.Array) {
					// This inspected element of 'processing_result' should
					// really be a CBOR Array at this point
				...
				if (nextPayload == null || session.getCurrentStep() != Constants.EDHOC_AFTER_M3) {
					// System.err.println("Inconsistent state before sending
					// EDHOC Message 3");
					Util.purgeSession(session, connectionId, edhocSessions, usedConnectionIds);
					client.shutdown();
					return;
			...
			if (nextPayload != null) {
				...
				if (requestType == Constants.EDHOC_MESSAGE_3) {
					...
					if (session.getApplicationProfile().getUsedForOSCORE() == true) {
						...
						/*
						 * Invoke the EDHOC-Exporter to produce OSCORE input
						 * material
						 */
						...
						// The Sender ID of this peer is the EDHOC connection
						// identifier of the other peer
						...
						if (Arrays.equals(senderId, recipientId)) {
							// System.err.println("Error: the Sender ID
							// coincides with the Recipient ID " +
						...
						try {
							...
							// System.err.println("Error when deriving the
						...
						try {
							...
							// System.err.println("Error when adding the OSCORE
							// Security Context to the context database "
							// + e.getMessage());
							Util.purgeSession(session, connectionId, edhocSessions, usedConnectionIds);
							client.shutdown();
							return;
					...
					// If the Error Message was generated while reading EDHOC
					// Message 2,
					// deliver EAD_2 to the application, if any was present in
					// EDHOC Message 2
				...
				try {
					...
					// OSCORE-protected request include the EDHOC option in the
					// request
					if (OSCORE_EDHOC_COMBINED == true && requestType == Constants.EDHOC_MESSAGE_3
						...
						// The combined request cannot be used if the Responder
						// has to send message_4
						if (session.getApplicationProfile().getUseMessage4() == true) {
							// System.err.println("Cannot send the combined
							// EDHOC+OSCORE request if message_4 is
							// expected\n");
							Util.purgeSession(session, connectionId, edhocSessions, usedConnectionIds);
							client.shutdown();
							return;
						...
						try {
							...
							// System.err.println("ConnectorException when
							// sending a protected request\n");
							Util.purgeSession(session, connectionId, edhocSessions, usedConnectionIds);
							client.shutdown();
							return;
						} catch (IOException e) {
							// System.err.println("IOException when sending a
							// protected request\n");
							Util.purgeSession(session, connectionId, edhocSessions, usedConnectionIds);
							client.shutdown();
							return;
						...
						if (myPayload != null) {
							...
							// Check if it is an EDHOC Error Message returned by
							// the server
							// when processing the combined EDHOC + OSCORE
							// request
							if (contentFormat == Constants.APPLICATION_EDHOC
								...
								responseType = MessageProcessor.messageType(myPayload, false, edhocSessions,
										connectionId, appProfile);

								if (responseType == Constants.EDHOC_ERROR_MESSAGE) {

									// System.err.println("Received an EDHOC
									// Error Message");
									CBORObject[] objectList = MessageProcessor.readErrorMessage(myPayload, cI,
											edhocSessions);
					...
					// System.err.println("ConnectorException when sending " +
					// myString + "\n");
					Util.purgeSession(session, connectionId, edhocSessions, usedConnectionIds);
					client.shutdown();
					return;
				...
				// Only an EDHOC message_4 or an EDHOC Error Message is
				// legitimate at this point
					...
					if (responsePayload == null)
						...
						if (responseType != Constants.EDHOC_ERROR_MESSAGE) {
							...
							if (responseType == Constants.EDHOC_MESSAGE_4) {
								if (expectMessage4 == false)
									discontinue = true;
								// Else it is fine, i.e. it is message_4 and it
								// is expected
							} else {
								// Any other message than message_4 and Error
								// Message
								// System.err.println("Received invalid reply to
								// EDHOC Message 3");
								discontinue = true;
							}
						...
						// It is an EDHOC Error Message
							...
							// System.err.println("Received an EDHOC Error
							// Message");
							CBORObject[] objectList = MessageProcessor.readErrorMessage(responsePayload, cI,
									edhocSessions);
					...
					// System.out.println("Determined EDHOC message type: " +
					// typeName + "\n");
					Util.nicePrint(typeName, responsePayload);

					if (responseType == Constants.EDHOC_MESSAGE_4) {
						processingResult = MessageProcessor.readMessage4(responsePayload, false, cI, edhocSessions,
								usedConnectionIds);

						if (processingResult.get(0) == null
								|| processingResult.get(0).getType() != CBORType.ByteString) {
							// System.err.println("Internal error when
							// processing EDHOC Message 4");
							Util.purgeSession(session, connectionId, edhocSessions, usedConnectionIds);
							client.shutdown();
							return;
						}

						// A non-zero length response payload would be an EDHOC
						// Error Message
						...
						if (nextMessage.length == 0) {
							...
							if (processingResult.size() == 2 && processingResult.get(1).getType() == CBORType.Array) {
								// This inspected element of 'processing_result'
								// should really be a CBOR Array at this point
								int length = processingResult.get(1).size();
							...
							// If message_4 was a Confirmable response, send an
							// empty ACK
						...
						// An EDHOC error message has to be returned in response
						// to EDHOC message_4
						else {

							// Deliver EAD_4 to the application, if any was
							// present in EDHOC Message 4
							if (processingResult.size() == 3 && processingResult.get(2).getType() == CBORType.Array) {
								// This inspected element of 'processing_result'
								// should really be a CBOR Array at this point
								int length = processingResult.get(2).size();
							...
							try {
								...
								// System.err.println("ConnectorException when
								// sending EDHOC Error Message");
								Util.purgeSession(session, connectionId, edhocSessions, usedConnectionIds);
								client.shutdown();
								return;
							} catch (IOException e) {
								// System.err.println("IOException when sending
								// EDHOC Error Message");
								Util.purgeSession(session, connectionId, edhocSessions, usedConnectionIds);
								client.shutdown();
								return;
				...
				// Send a request protected with the just established Security
				// Context
				...
				if (POST_EDHOC_EXCHANGE && usedForOSCORE == true) {
					...
					try {
						...
						// System.err.println("ConnectorException when sending a
						// protected request\n");
					} catch (IOException e) {
						// System.err.println("IOException when sending a
						// protected request\n");
	...
	private static void processErrorMessageAsResponse(CBORObject[] objectList, CBORObject connectionId) {
		...
		if (objectList != null) {
			...
			if (connectionId == null) {
				// System.err.println("Unavailable connection identifier to
				// delete EDHOC session");
				return;
			}
