...
public class EdhocClientTest {
	...
	/*
	 * Test execution of EDHOC.
	 * 
	 */
	@Test
	public void testEdhocClient() {
		...
		// Check that EDHOC successfully finishes
		assertEquals("Key establishment through EDHOC has failed", true, ret);
		
		// Check master secret length
		byte[] masterSecret = EdhocSession.getMasterSecretOSCORE(edhocExecutor.getEdhocSession());
		assertEquals(16, masterSecret.length);
		
		// Check master salt length
		byte[] masterSalt = EdhocSession.getMasterSaltOSCORE(edhocExecutor.getEdhocSession());
		assertEquals(8, masterSalt.length);
		...
		if (OSCORE_EDHOC_COMBINED) {
			...
			assertEquals("Hello World!", appResponseToCombinedRequest.getResponseText());
		}
