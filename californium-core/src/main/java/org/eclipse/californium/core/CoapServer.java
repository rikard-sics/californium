...
public class CoapServer implements ServerInterface {
	...
	public synchronized void start() {
		...
		org.eclipse.californium.elements.MyLogger.LOG_info("Starting server");
		...
		if (endpoints.isEmpty()) {
			...
			org.eclipse.californium.elements.MyLogger.LOG_info("no endpoints have been defined for server, setting up server endpoint on default port {}", port);
			CoapEndpoint.Builder builder = new CoapEndpoint.Builder();
			builder.setPort(port);
			builder.setNetworkConfig(config);
			addEndpoint(builder.build());
		...
		for (Endpoint ep : endpoints) {
			try {
				...
				org.eclipse.californium.elements.MyLogger.LOG_error("cannot start server endpoint [{}]", ep.getAddress(), e);
			}
		}
		if (started == 0) {
	...
	public synchronized void stop() {
		...
		if (running) {
			org.eclipse.californium.elements.MyLogger.LOG_info("Stopping server");
			for (Endpoint ep : endpoints) {
				ep.stop();
			}
			running = false;
	...
	public synchronized void destroy() {
		org.eclipse.californium.elements.MyLogger.LOG_info("Destroying server");
		...
		try {
			...
			org.eclipse.californium.elements.MyLogger.LOG_info("CoAP server has been destroyed");
			running = false;
		}
	}
