...
public final class UdpMatcher extends BaseMatcher {
	...
	public void sendRequest(final Exchange exchange) {
		...
		if (request.isObserve() && 0 == exchange.getFailedTransmissionCount()) {
			if (exchangeStore.assignMessageId(request) != Message.NONE) {
				...
				org.eclipse.californium.core.MyLogger.LOG_warn("message IDs exhausted, could not register outbound observe request for tracking");
		...
		try {
			if (exchangeStore.registerOutboundRequest(exchange)) {
				...
				org.eclipse.californium.core.MyLogger.LOG_debug("tracking open request [MID: {}, Token: {}]", request.getMID(), request.getToken());
			} else {
				org.eclipse.californium.core.MyLogger.LOG_warn("message IDs exhausted, could not register outbound request for tracking");
				request.setSendError(new IllegalStateException("automatic message IDs exhausted"));
			}
		...
		if (duplicate) {
			...
			if (!duplicate) {
				...
				if (exchangeStore.replacePrevious(idByMID, previous, exchange)) {
					org.eclipse.californium.core.MyLogger.LOG_debug("replaced request {} by new request {}!", previousRequest, request);
				} else {
					org.eclipse.californium.core.MyLogger.LOG_warn("new request {} could not be registered! Deduplication disabled!", request);
				}
			}
		}

		if (duplicate) {
			org.eclipse.californium.core.MyLogger.LOG_trace("duplicate request: {}", request);
			...
			previous.execute(new Runnable() {
				...
				public void run() {
					try {
						...
						org.eclipse.californium.core.MyLogger.LOG_warn("error receiving request {} again!", request, ex);
			...
			exchange.execute(new Runnable() {
				...
				public void run() {
					try {
						...
						org.eclipse.californium.core.MyLogger.LOG_warn("error receiving request {}", request, ex);
						receiver.reject(request);
					}
				}
			});
	...
	public void receiveResponse(final Response response, final EndpointReceiver receiver) {
		...
		org.eclipse.californium.core.MyLogger.LOG_trace("received response {}", response);
		...
		if (tempExchange == null) {
			...
			if (response.getType() != Type.ACK) {
					...
					prev.execute(new Runnable() {
						...
						public void run() {
							...
							if (prev.getCurrentRequest().isMulticast()) {
								org.eclipse.californium.core.MyLogger.LOG_debug("Ignore delayed response {} to multicast request {}", response,
							...
							try {
								if (endpointContextMatcher.isResponseRelatedToRequest(prev.getEndpointContext(),
									...
									org.eclipse.californium.core.MyLogger.LOG_trace("received response for already completed {}: {}", prev, response);
									response.setDuplicate(true);
									receiver.receiveResponse(prev, response);
									return;
								}
							} catch (RuntimeException ex) {
								org.eclipse.californium.core.MyLogger.LOG_warn("error receiving response {} for {}", response, prev, ex);
							}
				...
				org.eclipse.californium.core.MyLogger.LOG_trace("discarding unmatchable piggy-backed response from [{}]: {}", response.getSourceContext(),
						response);
			}
			return;
		}
		...
		exchange.execute(new Runnable() {
			...
			public void run() {
				...
				if (checkResponseToken && exchangeStore.get(idByToken) != exchange) {
					if (running) {
						org.eclipse.californium.core.MyLogger.LOG_debug("ignoring response {}, exchange not longer matching!", response);
					}
					return;
				}
				...
				if (context == null) {
					org.eclipse.californium.core.MyLogger.LOG_debug("ignoring response {}, request pending to sent!", response);
					return;
				}
				...
				try {
					if (endpointContextMatcher.isResponseRelatedToRequest(context, response.getSourceContext())) {
						...
						if (currentRequest.isMulticast()) {
							...
							org.eclipse.californium.core.MyLogger.LOG_debug("ignoring ACK, possible MID reuse before lifetime end for token {}, expected MID {} but received {}",
						...
						if (type != Type.ACK && !exchange.isNotification() && !currentRequest.isMulticast()
							...
							// request
							org.eclipse.californium.core.MyLogger.LOG_debug("ignoring notify for pending cancel {}!", response);
							return;
						}
						...
						if (type == Type.CON || type == Type.NON) {
							...
							if (exchangeStore.findPrevious(idByMID, exchange) != null) {
								org.eclipse.californium.core.MyLogger.LOG_trace("received duplicate response for open {}: {}", exchange, response);
								response.setDuplicate(true);
							}
						}
						...
						org.eclipse.californium.core.MyLogger.LOG_debug("ignoring potentially forged response for token {} with non-matching endpoint context",
								idByToken);
					}
				} catch (RuntimeException ex) {
					org.eclipse.californium.core.MyLogger.LOG_warn("error receiving response {} for {}", response, exchange, ex);
				}
				reject(response, receiver);
			}
		});
	...
	public void receiveEmptyMessage(final EmptyMessage message, final EndpointReceiver receiver) {
		...
		if (exchange == null) {
			org.eclipse.californium.core.MyLogger.LOG_debug("ignoring unmatchable empty message from {}: {}", message.getSourceContext(), message);
			return;
		}
		...
		exchange.execute(new Runnable() {
			...
			public void run() {
				if (exchange.getCurrentRequest().isMulticast()) {
					org.eclipse.californium.core.MyLogger.LOG_debug("ignoring {} message for multicast request {}", message.getType(), idByMID);
					return;
				}
				if (exchangeStore.get(idByMID) != exchange) {
					if (running) {
						org.eclipse.californium.core.MyLogger.LOG_debug("ignoring ack/rst {}, not longer matching!", message);
					}
					return;
				}
				try {
					if (endpointContextMatcher.isResponseRelatedToRequest(exchange.getEndpointContext(),
						...
						org.eclipse.californium.core.MyLogger.LOG_debug("received expected reply for message {}", idByMID);
					...
					org.eclipse.californium.core.MyLogger.LOG_warn("error receiving empty message {} for {}", message, exchange, ex);
				}
			}
		});
	}
	...
	private void reject(final Response response, final EndpointReceiver receiver) {
		...
		if (response.getType() != Type.ACK && response.hasMID()) {
			...
			org.eclipse.californium.core.MyLogger.LOG_debug("rejecting response from {}", response.getSourceContext());
