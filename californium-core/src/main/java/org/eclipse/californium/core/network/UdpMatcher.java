...
public final class UdpMatcher extends BaseMatcher {
	...
	public void sendRequest(final Exchange exchange) {
		...
		if (request.isObserve() && 0 == exchange.getFailedTransmissionCount()) {
			if (exchangeStore.assignMessageId(request) != Message.NONE) {
				...
				LOGGER.info("message IDs exhausted, could not register outbound observe request for tracking");
		...
		try {
			if (exchangeStore.registerOutboundRequest(exchange)) {
				...
				LOGGER.info("tracking open request [MID: {}, Token: {}]", request.getMID(), request.getToken());
			} else {
				LOGGER.info("message IDs exhausted, could not register outbound request for tracking");
				request.setSendError(new IllegalStateException("automatic message IDs exhausted"));
			}
		...
		if (duplicate) {
			...
			if (!duplicate) {
				...
				if (exchangeStore.findPrevious(idByMID, exchange) != null) {
					LOGGER.info("new request could not be registered!");
				}
			}
		}
			...
			previous.execute(new Runnable() {
				...
				public void run() {
					try {
						...
						LOGGER.info("error receiving request {} again!", request, ex);
			...
			exchange.execute(new Runnable() {
				...
				public void run() {
					try {
						...
						LOGGER.info("error receiving request {}", request, ex);
						receiver.reject(request);
					}
				}
			});
	...
	public void receiveResponse(final Response response, final EndpointReceiver receiver) {
		...
		if (tempExchange == null) {
			...
			if (response.getType() != Type.ACK) {
					...
					prev.execute(new Runnable() {
						...
						public void run() {
							...
							if (prev.getCurrentRequest().isMulticast()) {
								LOGGER.info("Ignore delayed response {} to multicast request {}", response,
							...
							try {
								if (endpointContextMatcher.isResponseRelatedToRequest(prev.getEndpointContext(),
									...
									LOGGER.info("received response for already completed {}: {}", prev, response);
									response.setDuplicate(true);
									receiver.receiveResponse(prev, response);
									return;
								}
							} catch (RuntimeException ex) {
								LOGGER.info("error receiving response {} for {}", response, prev, ex);
							}
				...
				LOGGER.info("discarding unmatchable piggy-backed response from [{}]: {}", response.getSourceContext(),
						response);
			}
			return;
		}
		...
		exchange.execute(new Runnable() {
			...
			public void run() {
				...
				if (context == null) {
					LOGGER.info("ignoring response {}, request pending to sent!", response);
					return;
				}
				...
				try {
					if (endpointContextMatcher.isResponseRelatedToRequest(context, response.getSourceContext())) {
						...
						if (currentRequest.isMulticast()) {
							...
							LOGGER.info("ignoring ACK, possible MID reuse before lifetime end for token {}, expected MID {} but received {}",
						...
						if ((response.getType() == Type.CON || response.getType() == Type.NON)
							...
							LOGGER.info("received duplicate response for open {}: {}", exchange, response);
							response.setDuplicate(true);
							...
							if (response.isNotification() && response.getType() != Type.ACK
								...
								LOGGER.info("ignoring notify for pending cancel {}!", response);
								return;
							}
							...
							if (exchangeStore.remove(idByMID, exchange) != null) {
								LOGGER.info("closed open request [{}]", idByMID);
							}
						}
					...
					LOGGER.info("error receiving response {} for {}", response, exchange, ex);
				}
				reject(response, receiver);
			}
		});
	...
	public void receiveEmptyMessage(final EmptyMessage message, final EndpointReceiver receiver) {
		...
		if (exchange == null) {
			LOGGER.info("ignoring unmatchable empty message from {}: {}", message.getSourceContext(), message);
			return;
		}
		...
		exchange.execute(new Runnable() {
			...
			public void run() {
				...
				if (exchangeStore.get(idByMID) != exchange) {
					if (running) {
						LOGGER.info("ignoring ack/rst {}, not longer matching!", message);
					}
					return;
				}
				try {
					if (endpointContextMatcher.isResponseRelatedToRequest(exchange.getEndpointContext(),
						...
						LOGGER.info("received expected reply for message {}", idByMID);
					...
					LOGGER.info("error receiving empty message {} for {}", message, exchange, ex);
				}
			}
		});
	}
	...
	private void reject(final Response response, final EndpointReceiver receiver) {
		...
		if (response.getType() != Type.ACK && response.hasMID()) {
			...
			LOGGER.info("rejecting response from {}", response.getSourceContext());
