...
public final class SweepDeduplicator implements Deduplicator {
	...
	private class SweepAlgorithm implements Runnable {
		...
		public void run() {
			try {
				org.eclipse.californium.elements.MyLogger.LOG_trace("Start Mark-And-Sweep with {} entries", incomingMessages.size());
				sweep();

			} catch (Throwable t) {
				org.eclipse.californium.elements.MyLogger.LOG_warn("Exception in Mark-and-Sweep algorithm", t);
			}
		}
			...
			if (!incomingMessages.isEmpty()) {
				...
				for (Map.Entry<?, DedupExchange> entry : incomingMessages.entrySet()) {
					...
					if ((exchange.nanoTimestamp - oldestAllowed) < 0) {
						...
						org.eclipse.californium.elements.MyLogger.LOG_trace("Mark-And-Sweep removes {}", entry.getKey());
						incomingMessages.remove(entry.getKey());
					}
				}
				org.eclipse.californium.elements.MyLogger.LOG_debug("Sweep run took {}ms", TimeUnit.NANOSECONDS.toMillis(ClockUtil.nanoRealtime() - start));
			}
		}
	}
}
