...
public final class InMemoryObservationStore implements ObservationStore {
	...
	public Observation putIfAbsent(Token key, Observation obs) {
		if (key == null) {
			...
			if (result == null) {
				org.eclipse.californium.elements.MyLogger.LOG_debug("added observation for {}", key);
			} else {
				org.eclipse.californium.elements.MyLogger.LOG_debug("kept observation {} for {}", result, key);
			}
			return result;
		}
	}
	...
	public Observation put(Token key, Observation obs) {
		if (key == null) {
			...
			if (result == null) {
				...
				org.eclipse.californium.elements.MyLogger.LOG_debug("replaced observation {} for {}", result, key);
			}
			return result;
		}
	}
	...
	public Observation get(Token token) {
		if (token == null) {
			...
			org.eclipse.californium.elements.MyLogger.LOG_debug("looking up observation for token {}: {}", token, obs);
	...
	public void remove(Token token) {
		if (token != null) {
			if (map.remove(token) != null) {
				org.eclipse.californium.elements.MyLogger.LOG_debug("removed observation for token {}", token);
			} else {
				org.eclipse.californium.elements.MyLogger.LOG_debug("Already removed observation for token {}", token);
			}
		}
	}
	...
	public synchronized void start() {
		...
		if (healthStatusInterval > 0 && HEALTH_LOGGER.isDebugEnabled() && executor != null) {
			statusLogger = executor.scheduleAtFixedRate(new Runnable() {
				...
				public void run() {
					if (enableStatus) {
						org.eclipse.californium.elements.MyLogger.LOG_debug("{} observes", map.size());
						Iterator<Token> iterator = map.keySet().iterator();
						int max = 5;
						while (iterator.hasNext()) {
							org.eclipse.californium.elements.MyLogger.LOG_debug("   observe {}", iterator.next());
							--max;
							if (max == 0) {
								break;
							}
