...
public class ReliabilityLayer extends AbstractLayer {
	...
	public ReliabilityLayer(NetworkConfig config) {
		...
		org.eclipse.californium.elements.MyLogger.LOG_info("ReliabilityLayer uses ACK_TIMEOUT={}, ACK_RANDOM_FACTOR={}, and ACK_TIMEOUT_SCALE={} as default",
	...
	public void sendRequest(final Exchange exchange, final Request request) {
		...
		org.eclipse.californium.elements.MyLogger.LOG_debug("{} send request, failed transmissions: {}", exchange, exchange.getFailedTransmissionCount());

		if (request.getType() == null) {
			request.setType(Type.CON);
		}
		if (request.getType() == Type.CON) {
			org.eclipse.californium.elements.MyLogger.LOG_debug("{} prepare retransmission for {}", exchange, request);
	...
	public void sendResponse(final Exchange exchange, final Response response) {
		...
		org.eclipse.californium.elements.MyLogger.LOG_debug("{} send response {}, failed transmissions: {}", exchange, response,
		...
		if (respType == null) {
			...
			org.eclipse.californium.elements.MyLogger.LOG_trace("{} switched response message type from {} to {} (request was {})", exchange, respType,
		...
		if (response.getType() == Type.CON) {
			org.eclipse.californium.elements.MyLogger.LOG_debug("{} prepare retransmission for {}", exchange, response);
	...
	private void prepareRetransmission(final Exchange exchange, final RetransmissionTask task) {
		...
		if (executor.isShutdown()) {
			org.eclipse.californium.elements.MyLogger.LOG_info("Endpoint is being destroyed: skipping retransmission");
			return;
		}
	...
	public void receiveRequest(final Exchange exchange, final Request request) {
		...
		if (request.isDuplicate()) {
			if (exchange.getSendNanoTimestamp() > request.getNanoTimestamp()) {
				...
				org.eclipse.californium.elements.MyLogger.LOG_debug("{}: {} duplicate request {}, server sent response delayed, ignore request", count,
						exchange, request);
				return;
			}
			...
			if (currentResponse != null) {
				...
				if (type == Type.NON || type == Type.CON) {
					...
					if (type == Type.CON) {
						...
						if (currentResponse.isAcknowledged()) {
							org.eclipse.californium.elements.MyLogger.LOG_debug("{} request duplicate: ignore, response already acknowledged!", exchange);
						} else {
							int failedCount = exchange.getFailedTransmissionCount() + 1;
							exchange.setFailedTransmissionCount(failedCount);
							org.eclipse.californium.elements.MyLogger.LOG_debug("{} request duplicate: retransmit response, failed: {}, response: {}",
				...
				org.eclipse.californium.elements.MyLogger.LOG_debug("{} respond with the current response to the duplicate request", exchange);
				...
				org.eclipse.californium.elements.MyLogger.LOG_debug("{} duplicate request was acknowledged but no response computed yet. Retransmit ACK",
						exchange);
				EmptyMessage ack = EmptyMessage.newACK(request);
				sendEmptyMessage(exchange, ack);

			} else if (exchange.getCurrentRequest().isRejected()) {
				org.eclipse.californium.elements.MyLogger.LOG_debug("{} duplicate request was rejected. Reject again", exchange);
				EmptyMessage rst = EmptyMessage.newRST(request);
				sendEmptyMessage(exchange, rst);

			} else {
				org.eclipse.californium.elements.MyLogger.LOG_debug("{} server has not yet decided what to do with the request. We ignore the duplicate.",
						exchange);
	...
	public void receiveResponse(final Exchange exchange, final Response response) {
		...
		if (response.getType() == Type.CON) {
			...
			if (response.isDuplicate()) {
				if (response.getNanoTimestamp() < exchange.getSendNanoTimestamp()) {
					...
					int count = counter.incrementAndGet();
					org.eclipse.californium.elements.MyLogger.LOG_debug("{}: {} duplicate response {}, server sent ACK delayed, ignore response", count,
							exchange, response);
					return;
				}
				...
				if (response.isRejected()) {
					ack = false;
					org.eclipse.californium.elements.MyLogger.LOG_debug("{} reject duplicate CON response, request canceled.", exchange);
				} else {
					org.eclipse.californium.elements.MyLogger.LOG_debug("{} acknowledging duplicate CON response", exchange);
				}
			} else {
				if (exchange.getRequest().isCanceled()) {
					ack = false;
					org.eclipse.californium.elements.MyLogger.LOG_debug("{} reject CON response, request canceled.", exchange);
				} else {
					org.eclipse.californium.elements.MyLogger.LOG_debug("{} acknowledging CON response", exchange);
				}
			}
			EmptyMessage empty;
			if (ack) {
		...
		if (response.isDuplicate()) {
			if (response.getType() != Type.CON) {
				org.eclipse.californium.elements.MyLogger.LOG_debug("{} ignoring duplicate response", exchange);
			}
		} else {
			exchange.getCurrentRequest().setAcknowledged(true);
	...
	public void receiveEmptyMessage(final Exchange exchange, final EmptyMessage message) {
		...
		if (message.getType() == Type.ACK) {
			org.eclipse.californium.elements.MyLogger.LOG_debug("{} acknowledge {} for {} {} ({} msg observer)", exchange, message, type, currentMessage,
					observer);
			currentMessage.setAcknowledged(true);
		} else if (message.getType() == Type.RST) {
			org.eclipse.californium.elements.MyLogger.LOG_debug("{} reject {} for {} {} ({} msg observer)", exchange, message, type, currentMessage, observer);
			currentMessage.setRejected(true);
		} else {
			org.eclipse.californium.elements.MyLogger.LOG_warn("{} received empty message that is neither ACK nor RST: {}", exchange, message);
			return;
		}
	...
	protected abstract class RetransmissionTask implements Runnable {
		...
		private void retry() {
			...
			try {
				exchange.setRetransmissionHandle(null);
				if (exchange.isComplete()) {
					org.eclipse.californium.elements.MyLogger.LOG_debug("Timeout: for {}, {}", exchange, message);
					return;
				}
				int failedCount = exchange.getFailedTransmissionCount() + 1;
				exchange.setFailedTransmissionCount(failedCount);

				org.eclipse.californium.elements.MyLogger.LOG_debug("Timeout: for {} retry {} of {}", exchange, failedCount, message);

				if (message.isAcknowledged()) {
					org.eclipse.californium.elements.MyLogger.LOG_trace("Timeout: for {} message already acknowledged, cancel retransmission of {}", exchange,
							message);
					return;

				} else if (message.isRejected()) {
					org.eclipse.californium.elements.MyLogger.LOG_trace("Timeout: for {} message already rejected, cancel retransmission of {}", exchange,
							message);
					return;

				} else if (message.isCanceled()) {
					org.eclipse.californium.elements.MyLogger.LOG_trace("Timeout: for {}, {} is canceled, do not retransmit", exchange, message);
					return;

				} else if (failedCount <= getReliabilityLayerParameters().getMaxRetransmit()) {
					org.eclipse.californium.elements.MyLogger.LOG_debug("Timeout: for {} retransmit message, failed: {}, message: {}", exchange, failedCount,
							message);
					...
					if (message.isCanceled()) {
						org.eclipse.californium.elements.MyLogger.LOG_trace("Timeout: for {}, {} got canceled, do not retransmit", exchange, message);
						return;
					}
					retransmit();
				} else {
					org.eclipse.californium.elements.MyLogger.LOG_debug("Timeout: for {} retransmission limit reached, exchange failed, message: {}", exchange,
							message);
					exchange.setTimedOut(message);
				}
			} catch (Exception e) {
				org.eclipse.californium.elements.MyLogger.LOG_error("Exception for {} in MessageObserver: {}", exchange, e.getMessage(), e);
			}
		}
