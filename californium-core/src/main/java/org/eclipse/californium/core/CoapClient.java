...
public class CoapClient {
	...
	public CoapClient useExecutor() {
		...
		executor.execute(new Runnable() {
			...
			public void run() {
				org.eclipse.californium.core.MyLogger.LOG_info("using a SingleThreadExecutor for the CoapClient");
			};
		});
		return this;
	}
	...
	public CoapClient setEndpoint(Endpoint endpoint) {
		...
		if (!endpoint.isStarted()) {
			try {
				endpoint.start();
				org.eclipse.californium.core.MyLogger.LOG_info("started set client endpoint {}", endpoint.getAddress());
			} catch (IOException e) {
				org.eclipse.californium.core.MyLogger.LOG_error("could not set and start client endpoint", e);
			}
		...
		if (executor == null) {
			...
			try {
				...
				if (!executor.isShutdown()) {
					org.eclipse.californium.core.MyLogger.LOG_warn("failed to execute job!");
				}
			}
		}
	}
			...
			execute(new Runnable() {
				...
				public void run() {
					try {
						...
						org.eclipse.californium.core.MyLogger.LOG_warn("exception while handling response", t);
					}
				}
			});
		}
			...
			execute(new Runnable() {
				...
				public void run() {
					try {
						...
						org.eclipse.californium.core.MyLogger.LOG_warn("exception while handling failure", t);
					}
				}
			});
		}
		...
		protected void deliver(CoapResponse response) {
			synchronized (relation) {
				if (relation.onResponse(response)) {
					...
					org.eclipse.californium.core.MyLogger.LOG_debug("dropping old notification: {}", response.advanced());
					return;
				}
			}
		}
