...
public final class Block2BlockwiseStatus extends BlockwiseStatus {
	...
	private static int determineResponseBlock2Szx(final Exchange exchange, final int preferredBlockSize) {
		if (exchange.getRequest() != null) {
			...
			if (block2 != null) {
				org.eclipse.californium.core.MyLogger.LOG_debug("using block2 szx from early negotiation in request: {}", block2.getSize());
				return block2.getSzx();
			}
		}
		org.eclipse.californium.core.MyLogger.LOG_debug("using default preferred block size for response: {}", preferredBlockSize);
			...
			if (block2 == null) {
				...
				if (etag != null) {
					...
					if (responseBlock.getOptions().getETagCount() != 1) {
						org.eclipse.californium.core.MyLogger.LOG_debug("response does not contain a single ETag");
						return false;
					} else if (!Arrays.equals(etag, responseBlock.getOptions().getETags().get(0))) {
						org.eclipse.californium.core.MyLogger.LOG_debug("response does not contain expected ETag");
						return false;
					}
				}
	...
	public static final void crop(final Response responseToCrop, final BlockOption requestedBlock) {
		...
		if (responseToCrop == null) {
			...
			org.eclipse.californium.core.MyLogger.LOG_debug("cropping response body [size={}] to block {}", bodySize, requestedBlock);
