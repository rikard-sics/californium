...
public class ObserveLayer extends AbstractLayer {
		...
		if (relation != null && relation.isEstablished()) {
			...
			if (exchange.getRequest().isAcknowledged() || exchange.getRequest().getType() == Type.NON) {
				...
				if (!ResponseCode.isSuccess(response.getCode())) {
					org.eclipse.californium.elements.MyLogger.LOG_debug("response has error code {} and must be sent as CON", response.getCode());
					response.setType(Type.CON);
					relation.cancel();
				} else {
					// Make sure that every now and than a CON is mixed within
					if (relation.check()) {
						org.eclipse.californium.elements.MyLogger.LOG_debug("observe relation check requires the notification to be sent as CON");
						response.setType(Type.CON);
					} else {
			...
			if (current != null && isInTransit(current)) {
				org.eclipse.californium.elements.MyLogger.LOG_debug("a former notification is still in transit. Postponing {}", response);
	...
	public void receiveResponse(final Exchange exchange, final Response response) {
		...
		if (response.isNotification() && exchange.getRequest().isCanceled()) {
			...
			org.eclipse.californium.elements.MyLogger.LOG_debug("rejecting notification for canceled Exchange");
	...
	private class NotificationController extends MessageObserverAdapter {
		...
		public void onAcknowledgement() {
			...
			relation.setNextControlNotification(null);
			if (next != null) {
				org.eclipse.californium.elements.MyLogger.LOG_debug("notification has been acknowledged, send the next one");
		...
		public void onRetransmission() {
			...
			final Response next = relation.getNextControlNotification();
			if (next != null) {
				org.eclipse.californium.elements.MyLogger.LOG_debug("notification has timed out and there is a fresher notification for the retransmission");
		...
		public void onTimeout() {
			ObserveRelation relation = exchange.getRelation();
			org.eclipse.californium.elements.MyLogger.LOG_info("notification for token [{}] timed out. Canceling all relations with source [{}]",
