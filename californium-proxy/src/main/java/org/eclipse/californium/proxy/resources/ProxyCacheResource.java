...
//			Request request = response.getRequest();
			...
			try {
				...
				org.eclipse.californium.elements.MyLogger.LOG_warn("Cannot create the cache key: {}", e.getMessage());
			}
...
//				Option maxAgeOption = response.getFirstOption(OptionNumberRegistry.MAX_AGE);
				...
				if (maxAgeOption != null) {
					...
					org.eclipse.californium.elements.MyLogger.LOG_debug("Updated cached response");
				} else {
					org.eclipse.californium.elements.MyLogger.LOG_warn("No max-age option set in response: {}", response);
				}
...
//							if (Bench_Help.DO_LOG) 
								org.eclipse.californium.elements.MyLogger.LOG_debug("Cached response");
						} else {
							org.eclipse.californium.elements.MyLogger.LOG_warn("Failed to insert the response in the cache");
						}
					} catch (ExecutionException e) {
						// swallow
						org.eclipse.californium.elements.MyLogger.LOG_warn("Exception while inserting the response in the cache", e);
					}
				} else {
				...
				org.eclipse.californium.elements.MyLogger.LOG_error("Code not recognized: {}", code);
			}
		}
	}
	...
	public Response getResponse(Request request) {
		...
		if (response != null) {
			org.eclipse.californium.elements.MyLogger.LOG_debug("Cache hit");
			...
			if (nanosLeft > 0) {
				...
				org.eclipse.californium.elements.MyLogger.LOG_debug("Expired response");

				// try to validate the response
				response = validate(cacheKey);
				if (response != null) {
					org.eclipse.californium.elements.MyLogger.LOG_debug("Validation successful");
				} else {
					invalidateRequest(cacheKey);
				}
			}
	...
	public void invalidateRequest(Request request) {
		...
		org.eclipse.californium.elements.MyLogger.LOG_debug("Invalidated request");
	}
	...
	private static final class CacheKey {
		...
		private static List<CacheKey> fromAcceptOptions(Request request) {
			...
			try {
				...
				org.eclipse.californium.elements.MyLogger.LOG_error("ISO-8859-1 encoding not supported: {}", e.getMessage());
			}
